...
DESCRIPTION:       The Interface file for Binary Tree's
...
class BinaryTreeInterface
{
   public:
      ...
      virtual bool remove(const dataType& data) = 0;
      ...
      virtual dataType getEntry(const dataType& anEntry) const = 0;
                       //throw(NotFoundException) = 0; 
                    // removed this method of throw exceptions
      ...
      virtual bool contains(const dataType& anEntry) const = 0;

      /** Traverses this binary tree in preorder (inorder, postorder) and
          calls the function visit once for each node.
       @param visit A client-defined function that performs an operation on
          or with the data in each visited node. */
      //virtual void preorderTraverse(void visit(dataType&)) const = 0;
      //virtual void inorderTraverse(void visit(dataType&)) const = 0;
      //virtual void postorderTraverse(void visit(dataType&)) const = 0;
